...
## Index selection and plotting layout
The lookup window has an index selector that combines a history list of recently used `peaks.duckdb` paths with a **Browse** action to pick a new file. Selecting an entry from the history list makes that index the active one immediately; browsing for a new file adds it to the history list and sets it as the active index. The active index is the one used for validation, lookup searches, and plot data retrieval.

The plotting area is a dual-plot layout: the top comparison plot overlays the currently selected preview spectra with any references moved into the right-hand plotting sidebar, while the bottom preview plot shows exactly one reference spectrum with its peak sticks and metadata. Right-clicking a reference in either sidebar updates the bottom preview panel immediately without changing the top comparison plot, so you can inspect candidates without disrupting the main overlay.
